...
# 題庫讀取
# 格式：
# Full sentence. | chunk 2 / chunk 1 / chunk 3
# ======================
...
def parse_question_line(line: str):
    ...
    if "|" not in line:
        raise ValueError(f"題目格式錯誤，缺少 | ：{line}")

    answer_part, chunk_part = line.split("|", 1)

    answer = answer_part.strip()
    chunks = [c.strip() for c in chunk_part.split("/") if c.strip()]

    if not answer:
        raise ValueError(f"正確答案為空：{line}")

    if len(chunks) < 2:
        raise ValueError(f"chunk 太少，至少需要 2 個：{line}")
    ...
    for level, count in quiz_plan.items():
        ...
        for idx in selected_indices:
            ...
            quiz.append({
                "level": level,
                "index": idx,
                "answer": banks[level][idx]["answer"],
                "chunks": banks[level][idx]["chunks"],
                "audio": audio_path,
            })
...
class PartBApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Versant Part B 練習器")
    ...
    def check_answer(self):
        ...
        correct_answer = self.quiz[self.current_index]["answer"]
        chunks = self.quiz[self.current_index]["chunks"]
