...
def parse_item(item: str):
    lines = [clean_line(line) for line in item.splitlines() if line.strip()]
...
class PartCApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Versant Part C 練習器")
    ...
    def check_answer(self):
        ...
        score = compare_answer(user_answer, reference_answer)

        dialogue_text = "\n".join(current["dialogue"])
